# ---------- Agent layer — OpenAI-powered with rule-based fallback ----------
...
call_openai <- function(system_prompt, user_prompt, max_tokens = 800L) {
  ...
  tryCatch({
    ...
    message("OpenAI response parse error: ", e$message)
    NULL
  })
}
...
normalize_alert_payload <- function(payload) {
  list(
    symbol = safe_chr(payload$symbol %||% payload$ticker %||% payload$asset, default = "SPY"),
    event_type = safe_chr(payload$event_type %||% payload$alert_name %||% payload$condition, default = "watchlist_signal"),
    source = safe_chr(payload$source %||% "tradingview", default = "tradingview"),
    trigger_price = safe_num(payload$trigger_price %||% payload$price %||% payload$close, default = NA_real_),
    message = safe_chr(payload$message %||% payload$description, default = "No message was provided in the webhook payload."),
    received_at = safe_time(payload$timestamp %||% payload$time %||% Sys.time(), default = Sys.time())
  )
}
...
risk_memo_agent <- function(alert, triage, risk, knowledge) {
  ...
  risk_memo_fallback(alert, triage, risk, knowledge)
}
