...
get_price_history <- function(symbol, lookback = DEFAULT_LOOKBACK) {
  ...
  result <- tryCatch({
    ...
    message("Yahoo Finance fetch failed for ", yahoo_sym, ": ", e$message, " — using simulated data.")
    generate_fallback_prices(asset, lookback)
  })

  assign(cache_key, list(data = result, fetched_at = Sys.time()), envir = .price_cache)
  result
}
...
historical_var_es <- function(price_df, window = 60L, conf = 0.95) {
  ...
  if (length(rets) < 10) {
    return(list(var_1d = NA_real_, es_1d = NA_real_, var_5d = NA_real_, es_5d = NA_real_))
  }
  losses <- -rets
  var_1d <- as.numeric(stats::quantile(losses, probs = conf, na.rm = TRUE))
  es_1d <- mean(losses[losses >= var_1d], na.rm = TRUE)
  scale_5d <- sqrt(5)
  list(var_1d = var_1d, es_1d = es_1d, var_5d = var_1d * scale_5d, es_5d = es_1d * scale_5d)
}
...
correlation_jump <- function(symbol, benchmark_symbol = "SPY", lookback_short = 20L, lookback_long = 60L) {
  ...
  short_corr <- stats::cor(utils::tail(merged$ret_lhs, lookback_short), utils::tail(merged$ret_rhs, lookback_short))
  long_corr <- stats::cor(utils::tail(merged$ret_lhs, lookback_long), utils::tail(merged$ret_rhs, lookback_long))
